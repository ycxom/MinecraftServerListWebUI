...
export async function updateUI(options = {}) {
    ...
    if (isRefresh && !skipAnimations) {
        // FLIP animation for refresh
        oldNodes.forEach(nodeEl => {
            oldPositions.set(nodeEl.id, nodeEl.getBoundingClientRect());
        });
    } else if (!isRefresh && oldNodes.length > 0 && !skipAnimations) {
        // Genie animation for group switch
        oldNodes.forEach(node => node.classList.add('genie-out'));
    ...
    groupsToDisplay.forEach(group => {
        ...
        group.nodes.forEach(node => {
            ...
            drawerDiv.className = `node-drawer ${animationClass} ${node.isOpen ? ' is-open' : ''}`;
    ...
    if (isRefresh && !skipAnimations) {
        ...
        // Genie cleanup
        setTimeout(() => {
            cleanupAnimationClasses(document.querySelectorAll('.node-drawer'), 'genie-in');
        }, 500);
    }
